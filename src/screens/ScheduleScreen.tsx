...
function agenteToBlock(bloque: BloqueAgente, index: number): ScheduleBlock {
  const inicio = new Date(bloque.fecha_inicio);
  const fin = new Date(bloque.fecha_fin);
  const h = inicio.getHours();
  const m = inicio.getMinutes();
  const horaInicio = `${h < 10 ? '0' : ''}${h}:${m < 10 ? '0' : ''}${m}`;
  const hf = fin.getHours();
  const mf = fin.getMinutes();
  const horaFin = `${hf < 10 ? '0' : ''}${hf}:${mf < 10 ? '0' : ''}${mf}`;
  const durationMin = Math.max((fin.getTime() - inicio.getTime()) / 60000, 15);
...
export default function ScheduleScreen() {
  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      // Fecha del día seleccionado esta semana
      const hoy = new Date();
      ...
      const response = await generarHorario(token, user.id_usuario, fecha, tareas);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]} edges={['top']}>
      ...
      <View style={styles.summaryStrip}>
        ...
        {bloquesAgente.length > 0 && (
          <View style={styles.summaryItem}>
            <View style={[styles.summaryDot, { backgroundColor: theme.primary }]} />
            <Text style={[styles.summaryText, { color: theme.textTertiary }]}>{bloquesAgente.length} del agente</Text>
          </View>
        )}
      </View>
          ...
          <View style={[styles.blocksColumn, { height: HOURS.length * HOUR_HEIGHT }]}>
            {todosBloques.length === 0 ? (
              <Text style={[styles.emptyText, { color: theme.textTertiary }]}>
                Sin actividades para este día — toca ✦ Generar
              </Text>
            ) : (
